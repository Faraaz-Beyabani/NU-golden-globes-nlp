...
ignore = ['golden', 'globes', 'globe', 'in', 'by', 'an', 'a', 'or', 'best', 'motion', 'picture', 'role', 'actor', 'actress', 'screenplay', 'comedy']
...
def get_presenters(tweets):
    ...
    for tweet in tweets:
        ...
        if ' present' in text:
            present_count[best_award]["present_tweets"] += 1
            if ' and ' in text:
                present_count[best_award]["present_plural"] += 1
            ...
            for i in range(len(tags)-1):
                ...
                if first_tag and sec_tag and first_word.lower() not in ignore and sec_word.lower() not in ignore:
                    maybe_name = f"{first_word} {sec_word}"
                    if maybe_name in present_count[best_award]["presenters"].keys():
                        present_count[best_award]["presenters"][maybe_name] += 1
                    else:
                        present_count[best_award]["presenters"][maybe_name] = 1
